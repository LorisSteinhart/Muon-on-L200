...
G4VParticleChange* RMGNeutronCaptureProcess::PostStepDoIt(const G4Track& aTrack, const G4Step& aStep) {
  ...
  G4cout << "Proposing own neutron capture result" << G4endl;
  ...
  for (const G4int energy : input.eg) {
    ...
    // G4int temp = secondary_gamma->GetTrackID();
    // secondary_gamma->SetTrackID(-temp);
    
    int initialMarker = 0; // Startwert für die Markierung
    MyTrackInformation* gammaInfo = new MyTrackInformation(initialMarker);
    secondary_gamma->SetUserInformation(gammaInfo);

    // Debug-Ausgabe zur Überprüfung
    G4cout << "Gamma nach NC an Gd erzeugt mit initialer Markierung: " 
           << gammaInfo->GetGenerationMarker() << ", Energie: " 
           << secondary_gamma->GetKineticEnergy() / keV << " keV" << G4endl;
